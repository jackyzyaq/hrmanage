...
function getHeader(begin_month,end_month){
	var _url = ctx+'/common/supervisorranking/queryHeaderResult.do';
	var param = {};
	param['state']='1';
	param['pageIndex']=1;
	param['pageSize']=1;
	param['type']=supervisor_header;
	param['begin_month']=begin_month+"-01 00:00:00";
	param['end_month']=end_month+"-01 00:00:00";
	ajaxUrl(_url,param,function(json){
		$("#supervisor_header").empty();
		$("#supervisor_header").append("<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" class=\"stdtable\"><thead><tr><th class=\"head1\"></th><th class=\"head1\"></th><th class=\"head1\"></th><th class=\"head1\"></th><th class=\"head1\"></th><th class=\"head1\"></th></tr></thead><tbody></tbody></table>");
		if(json.rows.length>0){
			...
			$.each(json.rows, function (n, value) { 
				...
				"<td>"+value.header_5+"</td>"+
				"<td>"+value.header_6+"</td>";
	        });
			$("#supervisor_header table tbody").append(_td+"</tr>");
			$("#supervisor_header table tbody").append("<tr>"+
					...
					"<td><input class='longinput' title=\"kpi_5\" type=\"text\" id='kpi_5' name='kpi_5' value=''/></td>"+
					"<td><input class='longinput' title=\"kpi_6\" type=\"text\" id='kpi_6' name='kpi_6' value=''/></td>"+
					"</tr>");
			$("#supervisor_header table tbody").append("<tr><td colspan=\"6\">"+
					"	<div class=\"stdformbutton\">"+
					"		<input type=\"hidden\" id=\"over_all\" name=\"over_all\" value=\"0\"/>"+
					"		<button id=\"supervisorRankingSubmit\" class=\"submit radius2\" onclick=\"addKPIData();\">提交</button>"+
			...
			$("#supervisor_header table tbody").append("<tr>"+
					...
					"<td><input class='longinput' title=\"header_5\" type=\"text\" id='header_5' name='header_5' value=''/></td>"+
					"<td><input class='longinput' title=\"header_6\" type=\"text\" id='header_6' name='header_6' value=''/></td>"+
					"</tr>");
			$("#supervisor_header table tbody").append("<tr><td colspan=\"6\">"+
					"	<div class=\"stdformbutton\">"+
					"		<button id=\"supervisorRankingHeaderSubmit\" class=\"submit radius2\" onclick=\"addHeader();\">提交</button>"+
					"	</div>"+
					"</td></tr>");
		}
	});
...
function addHeader(){
	if(validateForm("supervisor_header")){
		if (confirm('是否提交？')) {
			var param = getParamsJson("supervisor_header");
			param['begin_month'] = $("#begin_month").val().Trim()+"-01 00:00:00";
			param['end_month'] = $("#end_month").val().Trim()+"-01 00:00:00";
			ajaxUrl(ctx+'/common/supervisorranking/supervisorRankingAddHeader.do',param,function(json){
				if(json.flag=='1'){
				}else{
...
function addKPIData(){
	if(validateForm("supervisor_ranking_form")){
		if(overAll().Trim().length>0){
			showMsgInfo('数据格式不对！');
		}else{
			if (confirm('是否提交？')) {
				var param = getParamsJson("supervisor_ranking_form");
				param['begin_month'] = param['begin_month']+"-01 00:00:00";
				param['end_month'] = param['end_month']+"-01 00:00:00";
				ajaxUrl(ctx+'/common/supervisorranking/supervisorRankingAdd.do',param,function(json){
					if(json.flag=='1'){
					}else{
						showMsgInfo(json.msg+'');
...
function getEndMonthFn(){
	if($("#begin_month").val().Trim().length>0){
		...
		supervisor_ranking_view_inner();
	}
}

function supervisor_ranking_view_inner(){
	var params = {};
	params['begin_month'] = $("#begin_month").val().Trim()+"-01 00:00:00";
	params['end_month'] = $("#end_month").val().Trim()+"-01 00:00:00";
	ranking_inner(ctx+'/faurecia/ETOP5/number/data/supervisor_ranking_view.jsp',params,'supervisor_ranking_view');
}
function overAll(){
	var msg = '';
	if(!isNumeric($("#kpi_1").val())||
		...
		var overall  = 	((parseFloat($("#kpi_1").val())+
						...
						parseFloat($("#kpi_4").val())+
						parseFloat($("#kpi_5").val())+
						parseFloat($("#kpi_6").val()))/6).toFixed(2);
