...
<%
	...
	gapKPI.setStart_date(year+"-01-01");
	gapKPI.setOver_date(year+"-12-31");
	gapKPI.setExt_1(ext_1);
	gapKPI.setExt_2(ext_2);
	gapKPI.setDept_name(dept_name);
	List<GapKPI> list = gapKPIService.findMonthByCondition(gapKPI);
...
for(Integer month : monthData.keySet()){
	GapKPI ptmp = gapKPIService.queryByType(sdf2.parse(year+"-"+(month)+"-01"), kpi_type, dept_name, ext_1, ext_2);
	if(ptmp==null){
		target_flag = "-";
	}else{
		unit = (ptmp.getExt_8());
	...
	if(ext_3.equals("正向")){
		if((target_flag.equals("1")?((p==null)?0:p.getCum()):((p==null)?0:p.getActual()))>=((p==null)?0:p.getTarget())){
			health = (request.getContextPath()+"/images/"+Global.gap_kpi_health[0]+".png");
		}else{
			health = (request.getContextPath()+"/images/"+Global.gap_kpi_health[1]+".png");
		}
	...
	if(health.indexOf(Global.gap_kpi_health[0])>-1){
	...
	}else if(health.indexOf(Global.gap_kpi_health[1])>-1){
