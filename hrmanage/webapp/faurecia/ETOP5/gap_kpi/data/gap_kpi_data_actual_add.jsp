...
<%
	...
	List<GapKPI> listDept = gapKPIService.queryKPIDept();
%>
<script type="text/javascript">
	$(function(){
		$("#gap_kpi_actual_form #dept_name").change(function(){
			if($(this).val().length>0){
				loadKpiType('gap_kpi_actual_form #kpi_type',$(this).val());
			}
		});
		$("#gap_kpi_actual_form #kpi_date").focus(function(){
			if($(this).val().length>0){
				loadKpi($("#gap_kpi_actual_form #dept_name").val(),
					$("#gap_kpi_actual_form #kpi_date").val(),
					$("#gap_kpi_actual_form #kpi_type").val()
					);
			}
		});
		$("#gap_kpi_actual_form #kpi_type").change(function(){
			if($(this).val().length>0){
					loadKpi($("#gap_kpi_actual_form #dept_name").val(),
					$("#gap_kpi_actual_form #kpi_date").val(),
					$("#gap_kpi_actual_form #kpi_type").val()
		...
		$("#gap_kpi_actual_form #kpi_type").trigger("change");
		$("#gap_kpi_actual_form #dept_name").trigger("change");
		$("#gap_kpi_actual_form #kpi_date").trigger("focus");
	});
	function loadKpiType(obj,dept_name,kpi_type){
		var _url = ctx+"/faurecia/ETOP5/gap_kpi/data/load_kpi_type.jsp";
		var param = {};
	...
	function loadKpi(dept_name,kpi_date,kpi_type){
		var _url = ctx+"/faurecia/ETOP5/gap_kpi/data/load_kpi_object.jsp";
		var param = {};
		param['dept_name']=dept_name;
		param['kpi_date']=kpi_date;
		param['kpi_type']=kpi_type;
		ajaxUrl(_url,param,function(json){
			$("#gap_kpi_actual_form #target").val('');
			$.each(json.rows, function (n, value) {
				$("#gap_kpi_actual_form #target").val(value.target);
				$("#gap_kpi_actual_form #ext_3").val(value.ext_3);
				$("#gap_kpi_actual_form #ext_2").val(value.ext_2);
				$("#gap_kpi_actual_form #ext_1").val(value.ext_1);
				$("#gap_kpi_actual_form #ext_8").val(value.ext_8);
				$("#gap_kpi_actual_form #target_flag").val(value.target_flag);
	        });
		});
		$("#bigTypeInner").empty();
		var params = {};
        ...
        <tbody>
			...
			<tr>
				...
				<td>
					<select id="ext_1" name="ext_1" title="INPUT、OUTPUT" disabled="disabled" >
						...
						<option value="<%=Global.gap_kpi_io[0]%>"><%=Global.gap_kpi_io[0]%></option>
						<option value="<%=Global.gap_kpi_io[1]%>"><%=Global.gap_kpi_io[1]%></option>
					</select>
				</td>
				...
				<td>
					<select id="ext_2" name="ext_2" title="QCDP" disabled="disabled" >
						<option value="" selected>---Select---</option>
						<%for(String t:Global.gap_kpi_qcdp){ %>
            ...
            <tr>
				<td colspan="6">
					...
					<button id="gapKPISubmit" class="submit radius2">Submit</button>
					</div>
				</td>
            </tr>
        </tbody>
