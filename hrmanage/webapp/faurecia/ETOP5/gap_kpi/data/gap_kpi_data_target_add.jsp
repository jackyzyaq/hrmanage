...
<%
	...
	List<GapKPI> deptList = gapKPIService.queryKPIDept();
	if(deptList!=null&&!deptList.isEmpty()){
		for(GapKPI pk : deptList){
			deptStr += pk.getDept_name()+"|";
		}
		deptStr.substring(0,deptStr.length()-1);
	}
	
	List<GapKPI> kpiList = gapKPIService.queryKPIType();
	if(kpiList!=null&&!kpiList.isEmpty()){
		for(GapKPI pk : kpiList){
			kpiStr += pk.getKpi_type()+"|";
		}
		kpiStr.substring(0,kpiStr.length()-1);
	}
%>
<form id="gap_kpi_target_form" class="stdform" onSubmit="return false;">
