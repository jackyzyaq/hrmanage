...
<%
	...
	if(StringUtils.isEmpty(kpi_date)){
		...
		GapKPI gapKPI = gapKPIService.findMaxDayofMonth(kpi_type,year+"-01-01",year+"-12-31",dept_name,ext_1,ext_2);
		if(gapKPI==null){
			return ;
		}else{
			kpi_date = sdf.format(gapKPI.getKpi_date());
		}
	}
...
<script type="text/javascript">
	...
	function kpi_month_inner(){
		...
		var _url = ctx+'/faurecia/ETOP5/gap_kpi/kpi_month_inner.jsp';
		var params = {};
	...
	function kpi_day_inner(year,_month,kpi_type,kpi_date){
		...
		inner_html(ctx+'/faurecia/ETOP5/gap_kpi/kpi_day_inner.jsp?ext_1=<%=ext_1%>&ext_2=<%=ext_2%>&dept_name=<%=dept_name%>&kpi_date='+kpi_date+'&menu_id=<%=menu_id%>&year='+year+'&month='+_month+'&kpi_type='+kpi_type
