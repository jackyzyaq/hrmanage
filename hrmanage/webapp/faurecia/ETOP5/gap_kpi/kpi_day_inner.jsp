...
<%
	...
	GapKPI gapKPITmp = null;
	if(StringUtils.isEmpty(kpi_date)){
		gapKPITmp = gapKPIService.findMaxDayofMonth(kpi_type,year+"-"+month+"-01",year+"-"+month+"-"+Util.getMaxDay(Integer.valueOf(year),Integer.valueOf(month)),dept_name,ext_1,ext_2);
		if(gapKPITmp!=null){
			kpi_date = sdf.format(gapKPITmp.getKpi_date());
		}else{
			kpi_date = year+"-"+month+"-01";
		}
	}else{
		gapKPITmp = gapKPIService.queryByType(sdf.parse(kpi_date), kpi_type, dept_name, ext_1);
	}
	if(StringUtils.isEmpty(kpi_type)){
		return ;
	}
	
	GapKPI gapKPI = new GapKPI();
	gapKPI.setKpi_type(kpi_type);
	gapKPI.setStart_date(year+"-"+month+"-01");
	gapKPI.setOver_date(year+"-"+month+"-"+Util.getMaxDay(Integer.valueOf(year),Integer.valueOf(month)));
	gapKPI.setExt_1(ext_1);
	gapKPI.setExt_2(ext_2);
	gapKPI.setDept_name(dept_name);
	List<GapKPI> list = gapKPIService.findByCondition(gapKPI,null);
...
if(p!=null&&p.getKpi_date().getTime()<now.getTime()){
	if(StringUtils.isEmpty(p.getHealth_png())||p.getHealth_png().equals("-")){
		...
		if(p.getHealth_png().indexOf(Global.gap_kpi_health[0])>-1){
		...
		}else if(p.getHealth_png().indexOf(Global.gap_kpi_health[1])>-1){
...
function day_show(_v){
	showHtml('${ctx}/faurecia/ETOP5/gap_kpi/kpi_day_show.jsp?kpi_type=<%=kpi_type%>&dept_name=<%=dept_name%>&ext_1=<%=ext_1%>&ext_2=<%=ext_2%>&kpi_date=<%=year+"-"+month+"-"%>'+_v,'Detail',800,400);
}
</script>
<ul id="kpi_day_table" class="toplist">
	<li>
		...
		<%if(gapKPITmp!=null&&!StringUtils.isEmpty(gapKPITmp.getHealth_png())&&!gapKPITmp.getHealth_png().equals("-")){
			out.print("<img src='"+gapKPITmp.getHealth_png()+"' width='24' height='24'/>");
		}%>
		</div>
