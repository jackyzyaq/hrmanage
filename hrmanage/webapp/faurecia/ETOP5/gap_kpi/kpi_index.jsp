...
<%
	...
	String default_kpi_data = (""+Global.gap_kpi_io[1]+"|"+ext_2+"|"+kpi_type+"|"+dept_name+"|"+kpi_date+"");
%>
...
<script type="text/javascript">
	...
	function kpi_data_inner(){
		...
		var ext_2 = $("#ext_2").val();
		var ext_1 = $("#ext_1").val();
		var _url = ctx+'/faurecia/ETOP5/gap_kpi/kpi_data.jsp';
		var params = {};
...
<div>
		...
		out.print("<table style='width: 100%;height:20px;border: 2px solid "+Global.colors[2]+";'>");
		out.print("<tr><td style='text-align: center; vertical-align: middle;font-size: 16px;font-weight: bold;'>");
		out.print(""+(Global.gap_kpi_io[1])+"");
		out.print("</td></tr></table>");
		out.print("</div>");
		if(!StringUtils.isEmpty(ext_2)&&ext_1.toUpperCase().equals(Global.gap_kpi_io[1].toUpperCase())){
			tmpList = gapKPIService.getKPITypeAndDeptSql(ext_2, Global.gap_kpi_io[0]);
			for(int i=0;i<tmpList.size();i++){
				GapKPI p = tmpList.get(i);
...
<div>
		...
		if(!StringUtils.isEmpty(ext_2)&&ext_1.toUpperCase().equals(Global.gap_kpi_io[1].toUpperCase())){
			for(int i=0;i<tmpList.size();i++){
				GapKPI p = tmpList.get(i);
				String kpi_data = (""+Global.gap_kpi_io[0]+"|"+ext_2+"|"+p.getKpi_type()+"|"+p.getDept_name()+"|"+kpi_date+"");
				String kpi_name = p.getDept_name()+"<br/>"+p.getKpi_type()+"";
				GapKPI gapKPITmp = gapKPIService.queryByType(sdf.parse(kpi_date), p.getKpi_type(), p.getDept_name(), Global.gap_kpi_io[0], ext_2);
				color = (gapKPITmp==null||StringUtils.isEmpty(gapKPITmp.getHealth_png())||gapKPITmp.getHealth_png().equals("-")?Global.colors[2]:(gapKPITmp.getHealth_png().indexOf(Global.gap_kpi_health[0])>-1?Global.colors[0]:Global.colors[1]));
