...
<%
	...
	String default_kpi_data = (""+Global.plant_kpi_io[1]+"|"+ext_2+"|"+kpi_type+"|"+dept_name+"|"+kpi_date+"|/faurecia/ETOP5/plant_kpi/kpi_data.jsp");
%>
...
<div>
		...
		out.print("<table style='width: 100%;height:20px;border: 2px solid "+Global.colors[2]+";'>");
		out.print("<tr><td style='text-align: center; vertical-align: middle;font-size: 16px;font-weight: bold;'>");
		out.print(""+(Global.plant_kpi_io[1])+"");
		out.print("</td></tr></table>");
		out.print("</div>");
		if(!StringUtils.isEmpty(ext_2)&&ext_1.toUpperCase().equals(Global.plant_kpi_io[1].toUpperCase())){
			String start_date = kpi_date.split("-")[0]+"-"+kpi_date.split("-")[1]+"-01";
			String end_date = kpi_date.split("-")[0]+"-"+kpi_date.split("-")[1] + "-"+Util.getMaxDay(Integer.parseInt(kpi_date.split("-")[0]), Integer.parseInt(kpi_date.split("-")[1]));
			tmpList = departmentKPIService.queryByPlant(start_date, end_date, kpi_type, dept_name, ext_1, ext_2);
			for(int i=0;i<tmpList.size();i++){
				...
				out.print("<table style='width: 100%;height:20px;border: 2px solid "+Global.colors[2]+";'>");
				out.print("<tr><td style='text-align: center; vertical-align: middle;font-size: 16px;font-weight: bold;'>");
				out.print(""+(Global.plant_kpi_io[0])+"");
				out.print("</td></tr></table>");
				out.print("</div>");
			}
		}
...
<div>
		...
		if(!StringUtils.isEmpty(ext_2)&&ext_1.toUpperCase().equals(Global.plant_kpi_io[1].toUpperCase())){
			for(int i=0;i<tmpList.size();i++){
				DepartmentKPI p = tmpList.get(i);
				String kpi_data = (""+p.getExt_1()+"|"+ext_2+"|"+p.getKpi_type()+"|"+p.getDept_name()+"|"+kpi_date+"|/faurecia/ETOP5/department_kpi/kpi_data.jsp");
				String kpi_name = p.getDept_name()+"<br/>"+p.getKpi_type()+"";
				DepartmentKPI plantKPITmp = departmentKPIService.queryByType(sdf.parse(kpi_date), p.getKpi_type(), p.getDept_name(), Global.plant_kpi_io[0], ext_2);
				color = (plantKPITmp==null||StringUtils.isEmpty(plantKPITmp.getHealth_png())||plantKPITmp.getHealth_png().equals("-")?Global.colors[2]:(plantKPITmp.getHealth_png().indexOf(Global.department_kpi_health[0])>-1?Global.colors[0]:Global.colors[1]));
...
<div id="contentwrapper" style="margin-left: 10px;margin-right:10px;margin-bottom:5px;text-align:center;">
		...
		<input type="hidden" id="_url" name="_url" value="/faurecia/ETOP5/plant_kpi/kpi_data.jsp"/>
	<div class="clearall"></div>
	<div id="kpi_data" ></div>
</div>
</body>
