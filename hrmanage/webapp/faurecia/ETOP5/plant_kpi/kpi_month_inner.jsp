...
<%
	...
	for(Integer month:monthData.keySet()){
		monthData.put(month, plantKPIService.queryByType(sdf2.parse(year+"-"+month+"-"+Util.getMaxDay(Integer.parseInt(year), month)), kpi_type, dept_name, ext_1, ext_2));
	}
%>
...
for(Integer month : monthData.keySet()){
    ...
    if(ext_3.equals("正向")){
		if((actual)>=(target_v)){
			health = (request.getContextPath()+"/images/"+Global.plant_kpi_health[0]+".png");
		}else{
			health = (request.getContextPath()+"/images/"+Global.plant_kpi_health[1]+".png");
		}
	...
	if(health.indexOf(Global.plant_kpi_health[0])>-1){
		out.print("month_actual.push(["+month+", "+(p==null?"null":p.getCum())+"]),");
		out.print("month_unactual.push(["+month+", null]),");
	}else if(health.indexOf(Global.plant_kpi_health[1])>-1){
