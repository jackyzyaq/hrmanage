...
<%
	...
	String qrci_department_type = StringUtils.defaultIfEmpty(request.getParameter("qrci_department_type"), "");
	
	//ServletContext st = this.getServletConfig().getServletContext();
	//ApplicationContext ctx = WebApplicationContextUtils.getRequiredWebApplicationContext(st);
	//QRCIDataService qrciDataService = (QRCIDataService) ctx.getBean("qrciDataService");
	//QRCIData qrciData = qrciDataService.queryByQrciType(qrci_department_type);
...
<script>
	$(function(){
		var _url = ctx+'/common/qrci_department/queryResult.do';
		var param = {};
		param['pageIndex']=1;
		param['pageSize']=1000;
		param['qrci_department_type']='<%=qrci_department_type%>';
		valueIn(_url,param,'qrci_department_data_form',function(json){
			$.each(json.rows, function (n, value) {
		        	$.each(value,function(name,value) {
						$("#qrci_department_data_form #"+name).val(value.replaceAll('\\\|',"\n"));
					});
		        });
		});
		$("#qrciDataSubmit").click(function(){
			//if(validateForm("qrci_department_data_form")){
					if (confirm('是否提交？')) {
						ajaxUrl(ctx+'/common/qrci_department/qrciDepartmentDataEdit.do',validData({}),function(json){
							if(json.flag=='1'){
							}else{
			       ...
			       <table cellpadding="0" cellspacing="0" border="0" class="stdtable">
			            ...
			            <tbody>
			            	<tr>
			                    ...
			                    <td colspan="3">
			                    	<input class="longinput" title="<%=Global.qrci_department_head[0] %>" type="text" readonly="readonly"  id="<%=Global.qrci_department_head[0].replace("<br/>", "_").replace(" ", "_").replace("(", "").replace(")", "").replace("-", "_").replace("/", "_").toLowerCase() %>" name="<%=Global.qrci_department_head[0].replace("<br/>", "_").replace(" ", "_").replace("(", "").replace(")", "").replace("-", "_").replace("/", "_").toLowerCase() %>" value="<%=qrci_department_type %>" />
			                    </td>
			                ...
			                <tr>
			                    ...
			                    <td><%=Global.qrci_department_head[5] %>
