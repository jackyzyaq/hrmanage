...
<%
	...
	QRCIDepartmentDataService qrciDepartmentDataService = (QRCIDepartmentDataService) ctx.getBean("qrciDepartmentDataService");
	
	QRCIDepartmentData qrciDepartmentData = new QRCIDepartmentData();
	qrciDepartmentData.setState(1);
	List<QRCIDepartmentData> list = qrciDepartmentDataService.findByCondition(qrciDepartmentData,null);
%>
		<div class="widgetbox">
			<div>
			    <div class="widgetcontent padding0 statement">
			       <table cellpadding="0" cellspacing="0" border="0" class="stdtable">
			            ...
			            <thead>
			                <tr>
			                	<%for(String s:Global.qrci_department_head){ %>
			                    <th class="head0"><%=s %></th>
			                    <%} %>
			            ...
			            <tbody>
			            	...
			            	if(list!=null&&list.size()>0){ 
			            		for(int i=0;i<list.size();i++){
			            			QRCIDepartmentData pd = list.get(i);
			            	%>
			            	<tr id="qrci_department_data_<%=pd.getId() %>" onclick="qrci_department_data_click_select(this);" title="点击可修改">
			            		<td id="id" style="display: none"><%=pd.getId() %></td>
			                    <td id="qrci_department_type"><%=StringUtils.defaultIfEmpty(pd.getQrci_type(), "") %></td>
			                    ...
			                    <td id="department_manager"><%=StringUtils.defaultIfEmpty(pd.getDepartment_manager(), "") %></td>
		...
		<!--widgetbox-->
		<script>
		function qrci_department_data_click_select(obj){
			var valForm = $("#"+obj.id).find("*");
			$.each(valForm,function(i,v){
				if(v.id.length>0){
					var formVal = $(this).html().Trim();
					if(formVal.indexOf('<br')>-1){
						$("#qrci_department_data_form input[name='"+v.id+"']").each(function(index,value){
							//$(this).val(formVal.split("<br>")[index].Trim());
						});
					}else{
						//$("#qrci_department_data_form #"+v.id).val($(this).text().Trim());
					}
				}
			});
		}
