...
<%
	...
	List<DepartmentKPI> list = departmentKPIService.getKPITypeAndDeptSql(ext_2, ext_1);
	if(list!=null&&!list.isEmpty()){
		for(int i=0;i<list.size();i++){
		String objId = ext_2+"_"+i;
		DepartmentKPI pk = list.get(i);
		...
		pk = departmentKPIService.queryByType(sdf.parse(kpi_date), kpi_type, dept_name,ext_1,ext_2);
		String color = null;
		if(pk==null){
			pk = new DepartmentKPI();
			color = Global.colors[2];
		}else{
			color = (pk.getHealth_png().indexOf(Global.department_kpi_health[0])>-1?Global.colors[0]:(pk.getHealth_png().indexOf(Global.department_kpi_health[1])>-1?Global.colors[1]:Global.colors[2]));
		}		
%>
