...
<%
	...
	DepartmentKPI departmentKPITmp = null;
	if(StringUtils.isEmpty(kpi_date)){
		departmentKPITmp = departmentKPIService.findMaxDayofMonth(kpi_type,year+"-"+month+"-01",year+"-"+month+"-"+Util.getMaxDay(Integer.valueOf(year),Integer.valueOf(month)),dept_name,ext_1,ext_2);
		if(departmentKPITmp!=null){
			kpi_date = sdf.format(departmentKPITmp.getKpi_date());
		}else{
			kpi_date = year+"-"+month+"-01";
		}
	}else{
		departmentKPITmp = departmentKPIService.queryByType(sdf.parse(kpi_date), kpi_type, dept_name, ext_1);
	}
	...
	departmentKPI.setStart_date(year+"-"+month+"-01");
	departmentKPI.setOver_date(year+"-"+month+"-"+Util.getMaxDay(Integer.valueOf(year),Integer.valueOf(month)));
	departmentKPI.setExt_1(ext_1);
	departmentKPI.setExt_2(ext_2);
	departmentKPI.setDept_name(dept_name);
	List<DepartmentKPI> list = departmentKPIService.findByCondition(departmentKPI,null);
...
if(p!=null&&p.getKpi_date().getTime()<now.getTime()){
	if(StringUtils.isEmpty(p.getHealth_png())||p.getHealth_png().equals("-")){
		...
		if(p.getHealth_png().indexOf(Global.department_kpi_health[0])>-1){
		...
		}else if(p.getHealth_png().indexOf(Global.department_kpi_health[1])>-1){
...
function day_show(_v){
	showHtml('${ctx}/faurecia/ETOP5/department_kpi/kpi_day_show.jsp?kpi_type=<%=kpi_type%>&dept_name=<%=dept_name%>&ext_1=<%=ext_1%>&ext_2=<%=ext_2%>&kpi_date=<%=year+"-"+month+"-"%>'+_v,'Detail',800,400);
}
</script>
<ul id="kpi_day_table" class="toplist">
	<li>
		...
		<%if(departmentKPITmp!=null&&!StringUtils.isEmpty(departmentKPITmp.getHealth_png())&&!departmentKPITmp.getHealth_png().equals("-")){
			out.print("<img src='"+departmentKPITmp.getHealth_png()+"' width='24' height='24'/>");
		}%>
		</div>
