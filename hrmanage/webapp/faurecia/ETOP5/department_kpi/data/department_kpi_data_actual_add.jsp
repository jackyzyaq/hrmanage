...
<%
	...
	List<DepartmentKPI> listDept = departmentKPIService.queryKPIDept();
%>
<script type="text/javascript">
	$(function(){
		$("#department_kpi_actual_form #dept_name").change(function(){
			if($(this).val().length>0){
				loadKpiType('department_kpi_actual_form #kpi_type',$(this).val());
			}
		});
		$("#department_kpi_actual_form #kpi_date").focus(function(){
			if($(this).val().length>0){
				loadKpi($("#department_kpi_actual_form #dept_name").val(),
					$("#department_kpi_actual_form #kpi_date").val(),
					$("#department_kpi_actual_form #kpi_type").val()
					);
			}
		});
		$("#department_kpi_actual_form #kpi_type").change(function(){
			if($(this).val().length>0){
					loadKpi($("#department_kpi_actual_form #dept_name").val(),
					$("#department_kpi_actual_form #kpi_date").val(),
					$("#department_kpi_actual_form #kpi_type").val()
		...
		$("#department_kpi_actual_form #kpi_type").trigger("change");
		$("#department_kpi_actual_form #dept_name").trigger("change");
		$("#department_kpi_actual_form #kpi_date").trigger("focus");
	});
	function loadKpiType(obj,dept_name,kpi_type){
		var _url = ctx+"/faurecia/ETOP5/department_kpi/data/load_kpi_type.jsp";
		var param = {};
	...
	function loadKpi(dept_name,kpi_date,kpi_type){
		var _url = ctx+"/faurecia/ETOP5/department_kpi/data/load_kpi_object.jsp";
		var param = {};
		param['dept_name']=dept_name;
		param['kpi_date']=kpi_date;
		param['kpi_type']=kpi_type;
		ajaxUrl(_url,param,function(json){
			$("#department_kpi_actual_form #target").val('');
			$.each(json.rows, function (n, value) {
				$("#department_kpi_actual_form #target").val(value.target);
				$("#department_kpi_actual_form #ext_3").val(value.ext_3);
				$("#department_kpi_actual_form #ext_2").val(value.ext_2);
				$("#department_kpi_actual_form #ext_1").val(value.ext_1);
				$("#department_kpi_actual_form #ext_8").val(value.ext_8);
				$("#department_kpi_actual_form #target_flag").val(value.target_flag);
	        });
		});
		$("#bigTypeInner").empty();
		var params = {};
...
<form id="department_kpi_actual_form" class="stdform" onSubmit="return false;">
        ...
        <tbody>
			...
			<tr>
				...
				<td>
					<select id="ext_1" name="ext_1" title="INPUT、OUTPUT" disabled="disabled" >
						...
						<option value="<%=Global.department_kpi_io[0]%>"><%=Global.department_kpi_io[0]%></option>
						<option value="<%=Global.department_kpi_io[1]%>"><%=Global.department_kpi_io[1]%></option>
					</select>
				</td>
				...
				<td>
					<select id="ext_2" name="ext_2" title="QCDP" disabled="disabled" >
						<option value="" selected>---Select---</option>
						<%for(String t:Global.department_kpi_qcdp){ %>
            ...
            <tr>
				<td colspan="6">
					...
					<button id="departmentKPISubmit" class="submit radius2">Submit</button>
					</div>
				</td>
            </tr>
        </tbody>
