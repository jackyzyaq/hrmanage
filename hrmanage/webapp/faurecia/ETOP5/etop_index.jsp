...
<div class="bodywrapper">
	...
	$(function(){
		...
		if(<%=(i==0?"false":"true")%>){
			...
			common_frame_src("${ctx }"+defaultUrl+"&params=<%=params %>","iframe_menu_<%=i%>",<%=menuInfoMap.get(i).getId()%>);
		}else{
			$("#portal_right  #contentwrapper").empty();
		}
	});	
	function common_frame_src(url,name,etop5_menu_id){
		if(etop5_menu_id==200){
