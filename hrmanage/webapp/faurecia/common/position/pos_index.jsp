...
var pos_columns = [[
			 					...
			 					{field:'pos_code',title:'职位代码',width:200,sortable:true,align:'center'},
			 					{field:'pos_name',title:'职位名称',width:200,sortable:true,align:'center',hidden:false},
				...
				text:'<%=mi.getMenu_name()%>',
					...
					handler:function(){
						...
						<%}else if(mi.getMenu_code().indexOf("_edit_")>-1){%>
							...
							parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>"+"&pos_id="+rows[0].id,"编辑<%=mi.getMenu_name()%>");
						<%}else if(mi.getMenu_code().indexOf("_view_")>-1){%>
							...
							parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>"+"&pos_id="+rows[0].id,"查看<%=mi.getMenu_name()%>");
						<%}%>
					}
				},'-',
				<%
...
<body>
    ...
    <br />
	<jsp:include page="/faurecia/common/position/pos_search.jsp" />
	<br />    
	<div id="pos_datagrid"></div>
</body>
</html>