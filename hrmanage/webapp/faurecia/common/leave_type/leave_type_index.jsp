...
var leave_type_columns = [[
			 					...
			 					{field:'type_code',title:'编码',width:100,sortable:true,align:'center'},
			 					{field:'type_name',title:'名称',width:100,sortable:true,align:'center',hidden:false},
				...
				text:'<%=mi.getMenu_name()%>',
					...
					handler:function(){
						...
						<%}else if(mi.getMenu_code().indexOf("_edit_")>-1){%>
							...
							parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>"+"&leave_type_id="+rows[0].id,"编辑<%=mi.getMenu_name()%>");
						<%}else if(mi.getMenu_code().indexOf("_view_")>-1){%>
							...
							parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>"+"&leave_type_id="+rows[0].id,"查看<%=mi.getMenu_name()%>");
						<%}%>
					}
				},'-',
				<%
...
<body>
    ...
    <br />
	<jsp:include page="/faurecia/common/leave_type/leave_type_search.jsp" />
	<br />    
	<div id="leave_type_datagrid"></div>
</body>
</html>