	...
	$(function(){
		...
		$('#contentwrapper #submit').click(function(){
			...
			if(str_.Trim().length>0){
				...
				_html+=" <tr><td>"+(i+1)+"</td><td>第"+(step[i].split(",")[2].replace(']',''))+"步</td><td>"+(step[i].split(",")[1].split('|')[1])+"</td><td>"+(step[i].split(",")[1].split('|')[2])+"</td><td>"+(step[i].split(",")[1].split('|')[3])+"</td></tr>";
				}
				_html+="	        </tbody>"+
				"	    </table>"+
				"	</div><!--widgetcontent-->"+
				...
				parent.document.getElementById('step_info').value=str_;	
				parent.jClose();
			}
		});
	});
