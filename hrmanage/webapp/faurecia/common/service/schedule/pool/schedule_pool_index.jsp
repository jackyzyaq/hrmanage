...
var schedule_pool_columns = [[
			 					...
			 					{field:'operation',title:'操作',width:140,sortable:true,align:'center',
									formatter:function(value, rowData, rowIndex){
			 							var str = '';
			 							str+= '<a style="cursor:pointer;" onclick="var msg = addToSchedule(\''+rowData.id+'\');if(msg.Trim().length>0){parent.showMsgInfo(msg);}else{queryResult();}">进入审批流</a>&nbsp;&nbsp;';
			 							return str;
			 						},
				...
				text:'<%=mi.getMenu_name()%>',
					...
					handler:function(){
						...
						<%}else if(mi.getMenu_code().indexOf("_batch_inner_flow_")>-1){%>
							...
							parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>&"+param,"<%=mi.getMenu_name()%> ",1024);
						<%}%>
					}
				},'-',
				<%
