...
var total_columns = [[
	...
	{field:'wo_number',title:'单号',width:'120',align:'center',sortable:true,hidden:false,
		formatter:function(value, rowData, rowIndex){
			return "<a style='cursor:pointer;' onclick='parent.showHtml(\"${ctx}/faurecia/common/service/schedule/schedule_list_view.jsp?wo_number="+value+"\",\"排班\",1024);'>"+value+"</a>";
		}
	},
	...
	{field:'dept_names',title:'GAP',width:'180',align:'center',sortable:false,hidden:false}
			...
			var schedule_toolbar = [			
				...
				text:'<%=mi.getMenu_name()%>',
					...
					handler:function(){
						<%if(mi.getMenu_code().indexOf("_add_")>-1){%>
							...
							<%}else{%>
								parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>","排班<%=mi.getMenu_name()%>",1024);
							<%}%>
						<%}else if(mi.getMenu_code().indexOf("_edit_")>-1){%>
							...
							<%}else{%>
								...
								if(rows[0].status=='2'&&<%=!Util.contains(roleCodes, Global.default_role[1],",")%>){
									...
									parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>"+"&id="+rows[0].id,"排班<%=mi.getMenu_name()%> "+rows[0].wo_number,1024);
								}
							<%}%>
						...
						parent.showInfoCheck(function(r,s){
								...
								for(var i=0;i<rows.length;i++){
									...
									ajaxUrlFalse(ctx+'/common/scheduleInfo/scheduleCheck.do',param,function(json){
										if(json.msg!=''){
				...
				%>
				<%=Util.getHrClose(roleCodes,"/common/scheduleInfo/scheduleInvalid.do")%>
				{
					text:'刷新',
					iconCls:'icon-reload',
					handler:function(){
...
<script type="text/javascript" src="${ctx }/faurecia/common/service/schedule/js/schedule.js"></script>
</head>
<body>
	<jsp:include page="/share/jsp/menuAll.jsp" />
	<br />  
	<jsp:include page="/faurecia/common/service/schedule/schedule_search.jsp" >
		<jsp:param value="<%=isSH.toString() %>" name="isSH"/>
		<jsp:param value="<%=isAllEmp.toString() %>" name="isAllEmp"/>
	</jsp:include>
	<div id="schedule_datagrid"></div>
</body>
</html>