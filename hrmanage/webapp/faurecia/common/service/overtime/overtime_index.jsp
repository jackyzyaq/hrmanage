...
var overtime_columns = [[
								...
								{field:'create_date',title:'提交时间',width:140,sortable:true,align:'center'},
...
var total_columns = [[
	...
	{field:'wo_number',title:'单号',width:'120',align:'center',sortable:true,hidden:false,
		formatter:function(value, rowData, rowIndex){
			return "<a style='cursor:pointer;' onclick='parent.showHtml(\"${ctx}/faurecia/common/service/overtime/overtime_list_view.jsp?wo_number="+value+"\",\"排班\",1024);'>"+value+"</a>";
		}
	},
	...
	{field:'dept_names',title:'GAP',width:'180',align:'center',sortable:false,hidden:false}
			...
			var overtime_toolbar = [			
				...
				text:'<%=mi.getMenu_name()%>',
					...
					handler:function(){
						<%if(mi.getMenu_code().indexOf("_add_")>-1){%>
							...
							<%}else if(!employeeInfo.getType().equals(Global.employee_type[1])){%>
								parent.showInfo("您是“<%=Global.employee_type[0]%>”，没有权限！");
							<%}else{%>
								parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>","加班<%=mi.getMenu_name()%>",900);
							<%}%>
						<%}else if(mi.getMenu_code().indexOf("_edit_")>-1){%>
							...
							<%}else{%>
								...
								if(rows[0].status=='2'&&<%=!Util.contains(roleCodes, Global.default_role[1],",")%>){
									parent.showInfo('审批结束不能修改！');
									return false;
								}else{
									parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>"+"&id="+rows[0].id,"加班<%=mi.getMenu_name()%> "+rows[0].wo_number,900);
								}
							<%}%>
						...
						parent.showInfoCheck(function(r,s){
								...
								for(var i=0;i<rows.length;i++){
									...
									ajaxUrlFalse(ctx+'/common/overTimeInfo/overTimeCheck.do',param,function(json){
										if(json.msg!=''){
				...
				%>
				<%=Util.getHrClose(roleCodes,"/common/overTimeInfo/overTimeInvalid.do")%>
				{
					text:'刷新',
					iconCls:'icon-reload',
					handler:function(){
...
<body>
	...
	<br />  
	<jsp:include page="/faurecia/common/service/overtime/overtime_search.jsp" >
		<jsp:param value="<%=isSH.toString() %>" name="isSH"/>
		<jsp:param value="<%=isAllEmp.toString() %>" name="isAllEmp"/>
	</jsp:include>
	<div id="overtime_datagrid"></div>
</body>
</html>