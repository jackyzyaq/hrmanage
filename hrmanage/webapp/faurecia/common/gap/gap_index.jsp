...
var gap_columns = [[
			 					...
			 					{field:'gap_code',title:'编码',width:100,sortable:true,align:'center'},
			 					{field:'gap_name',title:'名称',width:100,sortable:true,align:'center',hidden:false},
				...
				text:'<%=mi.getMenu_name()%>',
					...
					handler:function(){
						...
						<%}else if(mi.getMenu_code().indexOf("_edit_")>-1){%>
							...
							parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>"+"&gap_id="+rows[0].id,"编辑<%=mi.getMenu_name()%>");
						<%}else if(mi.getMenu_code().indexOf("_view_")>-1){%>
							...
							parent.showHtml(ctx+"<%=mi.getUrl()%>?menu_id=<%=mi.getId()%>"+"&gap_id="+rows[0].id,"查看<%=mi.getMenu_name()%>");
						<%}%>
					}
				},'-',
				<%
...
<body>
    ...
    <br />
	<jsp:include page="/faurecia/common/gap/gap_search.jsp" />
	<br />    
	<div id="gap_datagrid"></div>
</body>
</html>