...
function fileQueued(file){
	addReadyFileInfo(file.id,file.name,"成功加载到<br/>上传队列");
}
...
function uploadProgress(file, bytesLoaded) {
	...
	try {
		...
		if (percent === 100) {
			...
			document.getElementById('okParam').disabled="disabled";
...
function uploadSuccess(file, serverData) {
	try {
		if(serverData==''){
			...
			addFileInfo(file.id,serverData);
		}
	} catch (ex) {
		this.debug(ex);
	}
...
function uploadComplete(file) {
	try {
		...
		document.getElementById('okParam').disabled="";
	} catch (ex) {
		this.debug(ex);
	}
}
