...
import com.yq.company.etop5.service.PipdReportService;
...
@Controller
@RequestMapping({"/common/pipdReport/*"})
public class PipdReportAction extends Common {

   private static final long serialVersionUID = -3979556978770262299L;
   private static final Logger logger = Logger.getLogger(PipdReportAction.class);
   private SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
   private SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
   @Resource
   private UserInfoService userService;
   @Resource
   private PipdReportService pipdReportService;
   @Resource
   private RoleInfoService roleInfoService;


   @RequestMapping({"queryResult.do"})
   public void queryResult(Page page, PipdReport pipdReport, HttpServletRequest request, HttpServletResponse response) throws Exception {
      ...
      try {
         List e = this.pipdReportService.findByCondition(pipdReport, (Page)null);
         ...
         List result = this.pipdReportService.findByCondition(pipdReport, page);
         sb.append("\'rows\':[");
         Iterator var9 = result.iterator();

         while(var9.hasNext()) {
            PipdReport json = (PipdReport)var9.next();
            sb.append("{");
   ...
   @RequestMapping({"pipdReportAdd.do"})
   public void pipdReportAdd(PipdReport pipdReport, HttpServletRequest request, HttpServletResponse response) {
      StringBuffer sb = new StringBuffer();

      try {
         String e = "";
         UserInfo user = Global.getUserObject(request);
         pipdReport.setOperater(Util.getOperator(user));
         pipdReport.setState(Integer.valueOf(1));
         this.pipdReportService.save(pipdReport);
         e = "操作成功！";
         sb.append("{");
         sb.append("\'msg\':\'" + e + "\'");
         sb.append("}");
   ...
   public PipdReportService getPipdReportService() {
      return this.pipdReportService;
   }

   public void setPipdReportService(PipdReportService pipdReportService) {
      this.pipdReportService = pipdReportService;
   }
